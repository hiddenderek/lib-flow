    ...
    channel.consume(q.queue, async (msg) => {
        if (msg) {
            try {
                ...
                logMessage(`Consuming event with name ${logType} (tracking id: ${content?.trackingId}) with data: ${JSON.stringify(content)}`, flowInfo)
                ...
                logError('Error consuming message', flowInfo, JSON.stringify(e))
                channel.nack(msg, undefined, false)
            }
        } else {
