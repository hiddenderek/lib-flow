...
export const askForAction = async(options: IAskForAction) : Promise<{status: number, data: Record<string, any> | undefined | null}>=> {
    ...
    console.log("MODE IN ACTION "  + options.flowMode)
    if (options.flowMode !== "resume") {
       ...
       logMessage(`flow '${options.meta?.flowId}' paused with 'askFor' action. Waiting for resume...`, flowLog)   
       return {status: 202, data: {schema: options.schema}}
    } else if (options.flowMode === "resume" && options.resumeWith) {
        logMessage(`Resuming flow '${options.meta?.flowId}'with 'askFor' action.`, flowLog)   
        ...
        if (schemaResult.valid) {
            logMessage(`flow '${options.meta?.flowId}' resumed with 'askFor' action. Resume data: ${JSON.stringify(resumeWith)}`, flowLog)   
        ...
        logMessage(`flow '${options.meta?.flowId}' has no resumeWith input for an 'askFor' action`, flowLog)   
